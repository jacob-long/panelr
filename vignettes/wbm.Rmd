...
They are fit via multilevel models which allow for some other nice possibilites
...
(z_i - \bar{z_i} = 0) + 
(\alpha_i - \bar{\alpha_i} = 0) + (\epsilon_{it} - \bar{e_i})
$$

By de-meaning everything, all the time-invariant variables drop out:

$$
y_{it} - \bar{y_i} = \mu_t + \beta_1(x_{it} - \bar{x_i}) + (\epsilon_{it} - \bar{e_i})
$$
...
$$
y_{it} = \beta_{0i} + \beta_1(x_{it} - \bar{x_i}) + (\epsilon_{it} - \bar{e_i})
$$
...
$$
xw_{it} = x_{it}w_{it} - \bar{xw}_i
$$

Where $\bar{xw}_i$ can also be expressed as $\frac{\sum_{t=1}^{T_i}{x_{it}w_{it}}}{T_i}$, the sum of all products for each
$i$ divided by the number of time points for each $i$, $T_i$.

[Giesselmann and Schmidt-Catran (2018)](https://ideas.repec.org/p/diw/diwwpp/dp1748.html)
...
```{r}
model <- wbm(lwage ~ wks + union + ms + occ | blk + fem | (t | id),
             use.wave = TRUE, data = wages)
summary(model)
```
...
the $\beta^+$ and $\beta^-$ coefficients as a formal wawy to assess the presence
...
You can see in the lower table that this differencee in coefficients is 
...
```{r message = TRUE}
model <- asym_gee(pov ~ mother + spouse + inschool + hours, data = teen,
                  family = binomial(link = "logit"), use.wave = TRUE, 
                  wave.factor = TRUE)
summary(model)
```
